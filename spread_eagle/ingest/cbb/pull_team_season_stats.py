...
def fetch_team_season_stats(season: int) -> List[Dict[str, Any]]:
    """
    Fetch all team season stats with pagination.
    """
    ...
    while True:
        ...
        resp = requests.get(
            f"{BASE_URL}/stats/team/season", headers=headers, params=params, timeout=60
        )
        ...
        new_ids = {r.get("teamId") for r in page if r.get("teamId") is not None}
...
def dedupe_by_team(records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    seen = set()
    out = []
    for r in records:
        tid = r.get("teamId")
...
def main() -> None:
    ...
    for year in range(args.start_year, args.end_year + 1):
        print(f"\n[{year}] Pulling team season stats...")
        stats = fetch_team_season_stats(year)
        stats = dedupe_by_team(stats)

        # Save individual year
        year_path = paths.raw / f"team_season_stats_{year}.json"
        with year_path.open("w", encoding="utf-8") as f:
            json.dump(stats, f, indent=2)

        print(f"  [OK] {year}: {len(stats):,} teams")
        all_years.extend(stats)

    # Save combined JSON
    all_path = paths.raw / f"team_season_stats_{args.start_year}_{args.end_year}.json"
    ...
    if all_years:
        ...
        csv_path = paths.raw / f"team_season_stats_{args.start_year}_{args.end_year}_flat.csv"
        df.to_csv(csv_path, index=False)
        print(f"\nSaved FLAT CSV -> {csv_path}")

    print(f"Saved ALL YEARS JSON -> {all_path}")
    print(f"Total: {len(all_years):,} team-seasons")

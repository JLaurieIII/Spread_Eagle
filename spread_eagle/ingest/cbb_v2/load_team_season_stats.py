"""
Team Season Stats loader - Aggregated team stats per season.
...
def load_team_season_stats(
    ...
    print("=" * 60)
    print(f"  TEAM SEASON STATS {'INCREMENTAL' if mode == 'incremental' else 'FULL'} LOAD ({years[0]}-{years[-1]})")
    print("=" * 60)
    ...
    for year in years:
        ...
        stats = fetch_with_params("/stats/team/season", {"season": year})
        ...
        print(f"    TOTAL: {len(stats):,} teams")

        # Save individual year file
        if write_files:
            year_path = paths.raw / f"team_season_stats_{year}_v2.json"
    ...
    print(f"\n  GRAND TOTAL: {len(all_stats):,} team-seasons")

    if not all_stats:
        return {"team_season_stats": 0, "status": "no_data", "mode": mode}

    # Write combined files
    if write_files:
        all_path = paths.raw / f"team_season_stats_{start_year}_{end_year}_v2.json"
        ...
        csv_path = paths.raw / f"team_season_stats_{start_year}_{end_year}_v2.csv"
    ...
    print("\n  DONE!")
    return {"team_season_stats": len(all_stats), "status": "success", "mode": mode}


if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="Load CBB team season stats")
